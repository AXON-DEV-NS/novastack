...
const langLib = {
    "ar-eg": { rate: 50, cur: "ج.م", calc: "🛠️ حاسبة المشاريع", pay: "💰 تأكيد الدفع", s1: "طلبك", s2: "تنفيذ", s3: "دفع", s4: "تسليم" },
    "en-us": { rate: 1, cur: "USD", calc: "🛠️ Project Calc", pay: "💰 Payment", s1: "Order", s2: "Work", s3: "Pay", s4: "Done" },
    "ar-sa": { rate: 3.75, cur: "ر.س", calc: "🛠️ حاسبة المشاريع", pay: "💰 تأكيد الدفع", s1: "طلبك", s2: "تنفيذ", s3: "دفع", s4: "تسليم" }
};

function updateUI() {
    const l = langLib[document.getElementById('lang-select').value] || langLib["ar-eg"];
        ...
        if(isAdmin) {
            ...
            onSnapshot(query(collection(db, "orders"), where("screenshot", "!=", "")), snap => {
                ...
                snap.forEach(d => {
                    ...
                    list.innerHTML += `<div style="background:#111; padding:8px; margin-top:5px; border-radius:10px; border-right:3px solid gold;">
                        ...
                        <span style="font-size:10px; color:#aaa;">${data.payTime}</span><br>
                        <button style="padding:4px; font-size:10px; cursor:pointer;" onclick="window.open('${data.screenshot}')">عرض السكرين 🖼</button>
                    </div>`;
                });
            });
        } else {
            ...
            onSnapshot(doc(db, "orders", user.email.replace(/\./g, "_")), snap => {
                if(snap.exists()) {
                    ...
                    for(let i=1; i<=4; i++) document.getElementById(`step-${i}`).classList.toggle('active', i <= sMap[d.status]);
...
document.getElementById('client-confirm-pay').onclick = async () => {
    ...
    if(!file) return alert("يرجى اختيار السكرين!");
    const btn = document.getElementById('client-confirm-pay');
